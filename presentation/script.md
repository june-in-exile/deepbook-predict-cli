...
> 場合:技術受眾簡報(開發者 / 技術團隊)
> 預計時長:約 16–19 分鐘
> 對應投影片:`present.html`(14 頁含 Live Demo 收尾;References 不另講)
...
在比較 DeepBook Predict 跟 Polymarket 之前,先花一分鐘把現有預測市場怎麼運作講清楚——「聽過名字」跟「真的搞懂」是兩回事。我用 Polymarket 為主軸,因為它是這個品類最大的代表。

走一遍 trader 視角的機制:

- **Step 1 · Deposit**——連錢包,把 USDC 押進去
- **Step 2 · 挑一個市場**——例如「BTC 年底前會不會 > $80,500?」。畫面上會有 Yes 跟 No 兩種 share,**它們的價格永遠加總到 $1**。譬如 Yes $0.42 / No $0.58,直接讀出來就是「市場認為事件成立的機率約 42%」
- **Step 3 · 下單**——Polymarket 用**訂單簿撮合**,背後是專業造市商在**每個市場**各自報價
- **Step 4 · 到期裁決**——resolution oracle(Polymarket 用 UMA optimistic oracle)判事件結果。事件成立,Yes 每股拿 $1、No 歸零;反之亦然

順帶一提:**Kalshi** 是同類產品,只是有 **CFTC 監管、結算中心化**;**Augur** 改用 **LMSR / AMM 曲線**報價、靠去中心化分叉裁決。定價跟結算機制各家略有不同,但**「每個市場各自為政」這個底層設計幾乎一致**。

**收尾一句話**:不管哪一家,每個「市場」都是**一個獨立的小 app**——自己的部位代幣、自己的訂單簿、自己的造市商。

這頁建立完整的 mental model,下一頁就直接打三個洞。
...
看完機制,問一個直接的問題:這個設計到底卡在哪?三件事,直接拆給你看。

- **痛點 1 · 流動性鎖在單一市場**
  - 每個市場自找造市商,熱門市場熱,長尾市場日均交易量常常 $100 都不到
  - Polymarket 開了上千個市場,大部分流動性極淺
  - 新市場的「**冷啟動**」是品類級難題——既貴又慢

- **痛點 2 · 部位是死代幣**
  - Yes share 就是 Yes share,你只能「持有」或「賣給下一個人」
...
**收尾橋接**:這三件事的共同根源都是同一個——**它們是 application,不是 chain primitive**。

下一頁先換個視角看「預測市場到底是什麼」,再用三頁把痛點的後果跟整個品類量化攤開,然後再來看 Predict 怎麼解。
...
需求那麼旺、選擇權卻上不來——這 $100M 的平坦 TVL,**正是前面 Slide 3 那三個架構痛點(流動性鎖死、部位死代幣、形狀只到 binary)在數字上的後果**。**架構問題,不是需求問題。**

Predict 要解的就是把這三件事一次解掉:**讓流動性共享、讓部位可組合、讓選擇權形狀成為原生原語。**

但在開始講「Predict 怎麼解」之前,先把一個最常見的誤解處理掉——下一頁。

---

## Slide 7 · Predict ≠ 傳統預測市場
...
Predict 是 primitive 不是 app——那它在 DeepBook 堆疊上是哪一塊?下一頁退一步看整個 stack。

---

## Slide 8 · DeepBook 三大原語
...
但這裡有個實作細節值得補一下:**Binary 其實是透過 vertical range 實作出來的**——兩者在合約層共用同一個 `RangeKey` 結構(`range_key.move:16-20`):
...
Binary up/down 是用 **sentinel 邊界**編碼出來的——`neg_inf = 0`、`pos_inf = u64::MAX`(`constants.move:46-49`):
...
**先看圖最上頭那一排——兩個 globals,一併叫 Predict:**
...
隱含波動率曲面: 把不同 strike 跟到期的隱含波動率畫成一張立體曲面,等於市場對「未來波動」的完整定價地圖,所有選擇權公允價都從這張曲面推出來。
...
**傳統鏈上選擇權**:LP / maker 寫一個部位就要鎖一筆抵押品 cover max payout,這筆錢綁死在那個市場直到結算——不能同時 cover 別的 strike、別的資產、別的到期。結果就是資本效率超差,每個新市場都得重新引導流動性。
...
- 整體節奏:前段(slide 2–7)鋪陳「現有預測市場 → 痛點 → 視角換成選擇權 → 量化 → 破除誤解」,**比較感性的 framing**;中段(8–11)是技術細節,**節奏放穩**;後段(12)是創新點 climax;13–14 拉回誠實感跟 demo。
- Slide 2 是「為觀眾建立 Polymarket 心智模型」,不要陷在 Kalshi / Augur 的細節辯論——這兩個只是輕點,證明「整個品類都長這樣」,**講完就過**。
- Slide 3 三痛點要**有節奏地一個一個攤開**——每個痛點先點原因再說後果;觀眾如果懂預測市場會點頭,不懂的會建立認知。
- Slide 4 是橋段,只有一句話,**講完就切下一頁**,不要在這頁停太久——它是給後面 payoff shapes 鋪框架用的。
- Slide 6 跟 Slide 3 是**呼應結構**——Slide 3 給質性痛點、Slide 6 給量化證據。講 Slide 6 時可以**回頭指 Slide 3**(「還記得剛剛那三個痛點嗎?$100M 平坦 TVL 就是它們的後果」)。
- Slide 7 是進入技術細節前的最後一次澄清——重點在「app vs primitive」這個區分,對照表只是輔助,**不要陷在六個 row 一個一個唸**;講完就接進 Slide 8 的三大原語堆疊。
- Slide 10 把原本的「資料流」併進來當文字補充——如果時間緊,可直接帶過「五步、< 400ms、binary 跟 range 對稱」,主力放在三條金流跟共用 Vault。
- 如果觀眾偏 LP / 金融,slide 12 多花 30 秒講提款限制器的設計。
- 如果觀眾偏 builder / SDK,slide 10 末段多花 30 秒講「binary 與 range 對稱路徑」的意義。
